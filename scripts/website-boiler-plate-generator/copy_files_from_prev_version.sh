...
parse_version() {
  ...
  if [ "${PATCH}" -ne 0 ]; then
    log "Invalid version format. Patch version must be 0. This script is only meant to copy contents for new minor version."
    exit 1
  fi
...
generate_new_content() {
  ...
  "${BOTTLEROCKET_WEBSITE_REPO_DIR}/scripts/software-versions-inventory/software-versions-inventory.sh" \
    "${BOTTLEROCKET_REPO_DIR}" "${BOTTLEROCKET_CORE_KIT_REPO_DIR}" > \
    "${package_dir}/index.markdown"
